...
function responsiveImageAttrs(image, sizes) {
  const responsiveSources = Array.isArray(image?.responsiveSources) ? image.responsiveSources : [];
  const srcset = responsiveSources
    .filter((source) => source?.url && Number(source.width) > 0)
    .map((source) => `${source.url} ${Number(source.width)}w`)
    .join(', ');
  return {
    src: image?.url,
    srcset: srcset || null,
...
function renderSharedHeader() {
  return `<a class="skip-link" href="#noi-dung-chinh">Đi đến nội dung chính</a><header class="site-header"><div class="container header-inner"><a class="brand" href="/#trang-chu" aria-label="RealView - Trang chủ"><span class="brand-mark" aria-hidden="true"><img src="/assets/realview-logo-v1.webp" alt="" width="128" height="75" /></span><span><b>REAL</b>VIEW</span></a><nav id="main-navigation" class="main-nav" aria-label="Điều hướng chính"></nav><div class="header-utilities"><a class="header-contact" href="/lien-he" aria-label="Mở trang Liên hệ RealView"><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M21 15a4 4 0 0 1-4 4H8l-5 3V7a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4v8Z" /><path d="M8 10h.01M12 10h.01M16 10h.01" /></svg><span>Liên hệ</span></a></div><button class="nav-toggle" type="button" aria-expanded="false" aria-controls="main-navigation"><span class="nav-toggle-label sr-only">Mở menu</span><span class="nav-toggle-lines" aria-hidden="true"><i></i><i></i></span></button></div></header>`;
}
...
export function renderBlogCard(value, options = {}) {
  ...
  const image = `<img${attrs({ ...responsiveImageAttrs(post.heroImage, '(max-width: 760px) calc(100vw - 32px), 480px'), loading: 'lazy', decoding: 'async' })}>`;
