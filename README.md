...
Để tạo nhanh file pool từ một danh sách dài API key, chạy `npm run generate:apify-pool`. Dán mỗi key trên một dòng, nhấn Enter ở dòng trống, chọn `replace` hoặc `append`, rồi nhập vị trí muốn lưu. Công cụ tự chia key thành từng nhóm 5★ → 1★, kiểm tra key trùng và từ chối danh sách không chia hết cho 5. File mặc định là `config/apify-pool.local.json`, được tạo với quyền chỉ tài khoản hiện tại đọc/ghi và đã nằm trong `.gitignore`.
