...
test('section đối ứng ẩn mặc định và chỉ có module nền riêng', async () => {
  ...
  assert.match(loader, /import\('\.\/counterpart-widget\.js\?v=9'\)/);
  assert.match(html, /counterpart-loader\.js\?v=9/);
  assert.match(script, /counterpart-widget\.css\?v=9/);
  ...
  assert.match(script, /M6\.5 8\.5h11l1 11h-13l1-11Z/);
  ...
  assert.match(styles, /\.counterpart-product-card--tiktok::before \{[^}]*height: 4px;[^}]*linear-gradient\(90deg, #25F4EE, #000000, #FE2C55\)/);
  assert.match(styles, /\.counterpart-platform-tag \{[^}]*top: 8px;[^}]*left: 8px;[^}]*display: inline-flex;/);
