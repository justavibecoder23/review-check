...
test('section đối ứng ẩn mặc định và chỉ có module nền riêng', async () => {
  ...
  const historyStyles = await readFile(new URL('public/history.css', root), 'utf8');
  ...
  assert.equal((`${html}\n${script}`.match(/d="M4 6h16"/g) || []).length, 2);
  ...
  assert.match(styles, /\.counterpart-actions \{[^}]*padding: 12px 24px;[^}]*display: flex;[^}]*justify-content: space-between;[^}]*background-color: #FFF8DE;/);
  ...
  assert.doesNotMatch(`${html}\n${script}\n${styles}`, /dockButton|has-counterpart|action-button--counterpart|counterpart-ready/);
  assert.match(historyStyles, /\.results-page \.result-action-bar \{ width: min\(1080px, calc\(100% - 30px\)\); grid-template-columns: 86px minmax\(0,1\.1fr\) minmax\(0,1fr\) minmax\(0,\.92fr\); \}/);
  assert.match(historyStyles, /@media \(max-width: 760px\)[\s\S]*\.results-page \.result-action-bar \{ display: none !important; \}/);
